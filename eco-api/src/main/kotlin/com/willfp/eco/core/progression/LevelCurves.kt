...
object LevelCurves {
     ...
     * @param freeFirstLevel Whether reaching `startLevel + 1` costs nothing. True for
     *                     EcoJobs and EcoPets, which prepend a zero to their list today.
        ...
        if (!requirements.isNullOrEmpty()) {
            ...
            for ((index, requirement) in requirements.withIndex()) {
                if (!requirement.isFinite() || requirement <= 0.0) {
                    ...
                    // would misname the level in every EcoSkills, EcoJobs and EcoPets warning.
