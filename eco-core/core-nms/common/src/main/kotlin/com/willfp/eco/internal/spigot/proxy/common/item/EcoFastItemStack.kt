...
class EcoFastItemStack(
    ...
    override fun removeItemFlags(vararg hideFlags: ItemFlag) {
        var tooltip = handle.get(DataComponents.TOOLTIP_DISPLAY) ?: return
        for (flag in hideFlags) {
            tooltip = when (flag) {
                ItemFlag.HIDE_ENCHANTS -> tooltip.withHidden(DataComponents.ENCHANTMENTS, false)
                ItemFlag.HIDE_ATTRIBUTES -> tooltip.withHidden(DataComponents.ATTRIBUTE_MODIFIERS, false)
                ItemFlag.HIDE_UNBREAKABLE -> tooltip.withHidden(DataComponents.UNBREAKABLE, false)
                ItemFlag.HIDE_DESTROYS -> tooltip.withHidden(DataComponents.CAN_BREAK, false)
                ItemFlag.HIDE_PLACED_ON -> tooltip.withHidden(DataComponents.CAN_PLACE_ON, false)
                ItemFlag.HIDE_ADDITIONAL_TOOLTIP -> TooltipDisplay(false, tooltip.hiddenComponents)
                ItemFlag.HIDE_DYE -> tooltip.withHidden(DataComponents.DYED_COLOR, false)
                ItemFlag.HIDE_ARMOR_TRIM -> tooltip.withHidden(DataComponents.TRIM, false)
                ItemFlag.HIDE_STORED_ENCHANTS -> tooltip.withHidden(DataComponents.STORED_ENCHANTMENTS, false)
            }
        }
        handle.set(DataComponents.TOOLTIP_DISPLAY, tooltip)
        apply()
    ...
    override fun hasItemFlag(flag: ItemFlag): Boolean {
        ...
        return when (flag) {
            ItemFlag.HIDE_ENCHANTS -> !tooltip.shows(DataComponents.ENCHANTMENTS)
            ItemFlag.HIDE_ATTRIBUTES -> !tooltip.shows(DataComponents.ATTRIBUTE_MODIFIERS)
            ItemFlag.HIDE_UNBREAKABLE -> !tooltip.shows(DataComponents.UNBREAKABLE)
            ItemFlag.HIDE_DESTROYS -> !tooltip.shows(DataComponents.CAN_BREAK)
            ItemFlag.HIDE_PLACED_ON -> !tooltip.shows(DataComponents.CAN_PLACE_ON)
            ItemFlag.HIDE_ADDITIONAL_TOOLTIP -> tooltip.hideTooltip
            ItemFlag.HIDE_DYE -> !tooltip.shows(DataComponents.DYED_COLOR)
            ItemFlag.HIDE_ARMOR_TRIM -> !tooltip.shows(DataComponents.TRIM)
            ItemFlag.HIDE_STORED_ENCHANTS -> !tooltip.shows(DataComponents.STORED_ENCHANTMENTS)
        }
    }
